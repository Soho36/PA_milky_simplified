...
**Execution: blocked copying.** Each funded account holds at most one position. A signal is copied by every live account that is flat at its entry; an account still in an earlier trade skips it. Everything else matches the [non-blocking comparison](../../../comparisons/legacy_25k_vs_50k/reserve_by_policy/reserve_by_policy__REPORT.md), where every live account copies every signal: same tape, grid, budgets, fees and rules.
...
- [Research contract and evidence](study.json). [Operating notes](reserve_by_policy__HOW_THIS_STUDY_WORKS.md).
