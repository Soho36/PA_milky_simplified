...
SOURCE=purchases.study_path(STUDY, 'purchases')/'study.json'
...
def main():
    ...
    chosen=[p for p in purchases.study_policies(STUDY) if p.name in STUDY['cadence']['policy_names']]
    jobs=[]
    for initial,monthly in BUDGETS:
        a=AcquisitionPolicy('monthly_one',initial,monthly,max_live_accounts=STUDY['max_live_accounts'])
        for p in chosen:
            for c in STUDY['cadence']['cadences']:
                levels=STUDY['cadence']['retained_balances']+([STUDY['cadence']['minimum_extra_retained_balance']] if p.amount_rule=='minimum' else [])
                for level in levels:
    ...
    if '--refresh-purchase-tables' in sys.argv:
        ...
        for row, (_, acquisition, policy) in zip(rows, jobs):
            assert row['acquisition_config']==asdict(acquisition), 'Funding/cap changed; rerun simulations'
            assert row['withdrawal_config']==policy.to_payload(), 'Cadence grid changed; rerun simulations'
    else:
        with ProcessPoolExecutor(max_workers=STUDY['workers'],initializer=purchases.initialize) as pool:
    ...
    assert len(checks)==len(BUDGETS)*len(purchases.withdrawal_choices())
    for r in rows:
    ...
    payload={'schema':'pa_milky.budget_cadence_study.v1','generated_utc':datetime.now(timezone.utc).isoformat(),'study_profile':purchases.profile_provenance(STUDY),'config':to_payload(purchases.C),'inputs':input_digest(purchases.C),'engine':engine_digest(),'runner_sha256':sha256_file(Path(__file__)),'evaluator_sha256':sha256_file(Path(purchases.__file__)),'design':{'budgets':BUDGETS,'max_live_accounts':STUDY['max_live_accounts'],'purchase_policy':'monthly_one','funding_starts':'second calendar month','selection':'configured cadence grid per budget; no fresh cushion optimization','terminal':'one permitted request; no reinvestment of terminal receipts'},'purchase_source_sha256':sha256_file(SOURCE),'shared_controls':checks,'rows':rows}
    ...
    report=f"# Withdrawal cadence: monthly purchases, {STUDY['max_live_accounts']} live accounts, matched cash budgets\n\n"
    report+=f'{len(rows)} candidates: configured cadence/cushion settings under each of {len(BUDGETS)} budgets. Accounts means total purchased across the dataset; Alive means survivors before terminal withdrawal.\n\n'
    report+=f'All {len(checks)} shared monthly-purchase controls exactly reproduce the purchase study across cash, account counts, funding diagnostics and economic ledgers. Only policy display names differ.\n\n'
    ...
    for initial,monthly in BUDGETS:
        ...
        for score,title in [('ongoing_net_cash_usd','Ongoing cash'),('combined_net_cash_usd','Terminal-inclusive cash')]:
            leaders=[max([r for r in family if r['withdrawal_policy']==p.name and r['withdrawal_config']['cadence']==c],key=lambda r:(r[score],r['ongoing_net_cash_usd'])) for p in chosen for c in STUDY['cadence']['cadences']]
