"""Independent graph/set audit of cross-strategy RR/GG episodes and pair counts."""
...
def main():
    ...
    for months in ('1','3','6'):
        for budget in ('1500','6800'):
            native=[r for r in events if r['months']==months and r['budget']==budget]
            for gap in (5,20,40):
    ...
    for p in pairs:
        ...
        for field,value in dict(periods=len(valid),left_failed=len(left),right_failed=len(right),both=len(left&right),
