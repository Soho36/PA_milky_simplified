...
GUIDES = {
    ...
    COMPARE_BLOCKED: ('Which account size and withdrawal/replacement approach works best when each account holds one position?',
        'All seven studies have been rerun. Blocking lowers cash in most matched settings. With instant supply the winners keep their shape (prompt replacement, maximum withdrawals, little or no reserve) and 25K leads 50K in every funded budget. Once supply is limited, winners switch to minimum withdrawals and buy far fewer accounts; with evaluations traded on the tape, 50K leads 25K in three of the four budgets and in the main pipeline search. Under daily maximum withdrawals the March 2026 cliff moves from $6,780 to $6,100 and daily-maximum cash roughly halves.',
        'Each blocked study reads its predecessor from this tree, never from the non-blocking one, and reproduces it where their settings coincide. Compare any folder with the same folder under comparisons.'),
    ...
    COMPARE_BLOCKED+'/eval_supply': ('Can evaluations traded on the strategy supply enough replacement PAs when each account holds one position?',
        'Across 4,912 settings run in both modes, blocking lowers total cash in 3,030 and raises it in 1,478; the other 404 lose the seed either way. Best tested totals fall 19–25% for every product and budget. Evaluation supply removes the churn: 25K with $1,000 + $200/month earns $445,315 with 66 funded PAs, against $665,421 and 531 PAs with instant $200 seats. 50K now leads 25K in three of the four budgets; 25K leads only with $1,000 and no contributions. All eight winners keep five spares, seven take the minimum checked monthly, and a funded PA costs about $186–$266.',
        'Evaluations trade one position at a time in both trees, so the EODMAE check is identical to the non-blocking study. 123 settings shared with the 25K blocked-pipeline study reproduce it exactly, including per-trade account assignments. The short-month columns are not comparable with the saved non-blocking study: a later engine change altered how supply-limited days are logged.'),
    COMPARE_BLOCKED+'/pipeline_capacity': ('Is replacement capacity limiting profit when each account holds one position, and does a larger or staggered evaluation pipeline help?',
        'Best tested winners fall 19–24% against the non-blocking pipeline study, and every winner now takes the minimum withdrawal with $0–$2,400 above the floor. In the main shared-seat comparison 50K leads 25K: $518,683 against $497,570 total, and $469,400 against $453,544 ongoing; without blocking 25K led both. Expanded pipelines lift the old aggressive instant-supply policy only to $325,884 (25K) and $335,443 (50K), about half its instant-supply cash. The best daily-maximum cushion is still $6,800 for both products.',
        'shared_seats reserves one of 20 places for each live PA, activated spare or in-flight evaluation; evals_outside_cap is a separate sensitivity. Anchors and controls come from the blocked eval_supply and reserve_by_policy studies, and all 16 controls reproduce, including per-trade account assignments. The capacity chart is not drawn here: it needs ReportLab, which the venv lacks.'),
    COMPARE_BLOCKED+'/reserve_frontier': ('With daily maximum fixed and each account holding one position, where is the reserve survival boundary and how much cash does extra cushion defer?',
        'With the pipelines frozen as in the non-blocking study, the March 30, 2026 trade no longer kills the book: no PA dies that day from $6,100 for either product, against $6,780.11 without blocking, and at $6,700 all 20 PAs survive it. Blocked PAs reach that trade with $696–$810 of profit equity instead of $101. Daily-maximum cash is about half the non-blocking level: at $6,800 total net is $312,331 (25K) and $315,651 (50K), against $629,920 and $641,765. The best ongoing reserve drops to $4,000 (25K) and $1,000 (50K). Across annual cold starts, aged-PA deaths stop from $6,100–$6,200; in the 50K full history, four PAs at least a year old still die even at $10,000.',
        'Pipelines, reserve grid, $10 and cent-level probes and windows are frozen from the non-blocking study, where the pipelines came from its shared-seat daily-maximum search, so only the copying rule differs. Six saved pipeline rows reproduce, including per-trade assignments, and no replayed PA held two trades at once. The chart is not drawn (ReportLab is missing), and the non-blocking folder’s one-off validation files have no counterpart.'),
    COMPARE_BLOCKED+'/march_failure_review': ('Does minimum withdrawal reduce synchronized failure when each account holds one position, and did replacements really restore the book quickly?',
        'The March 30, 2026 trade kills no PA in any of the 14 replays: blocked books enter it with $696–$810 of profit equity instead of $101, so the non-blocking cliff came from stacked overlapping losses. Synchronized failure remains at lower reserves — the $5,700 daily-maximum books lose all 20 PAs together on March 26, and the daily-minimum winners lose theirs on March 5–6, ending with 10 (25K) and 16 (50K). The $6,700 maximum books keep earning after the event, $18,933 and $19,047, where the non-blocking ones received nothing.',
        'Reserves and pipelines are frozen as in the non-blocking review; the two minimum winners come from the blocked pipeline study and reproduce it, including per-trade account assignments. Four daily-maximum books are unchanged by the evaluation pause, against two without blocking. The minimum runs receive no closing withdrawal: their PAs had just been paid, so the firm’s minimum-trading-days rule denies the request.'),
    COMPARE_BLOCKED+'/reserve_frontier_minimum': ('Does daily minimum improve the reserve cash/survival trade-off when each account holds one position?',
        'Blocking lowers total cash in 880 of the 1,280 settings run in both modes and raises it in 400. Minimum still leads on best tested ongoing cash: $329,932 at $3,500 against $319,487 at $4,000 for 25K, and $440,140 at $3,000 against $337,153 at $1,000 for 50K. At $6,700–$6,800 both rules keep all 20 PAs, where non-blocking maximum lost most of its book at $6,700. Blocking itself spreads balances — the mean spread among frozen-floor PAs is two to three times larger — so maximum is no longer synchronized. Aged-PA deaths stop from $6,400 under minimum against $6,200 under maximum for 25K, and at no tested reserve for 50K.',
        'Same grid, $5,700 probe, pipelines and windows as the non-blocking study. 416 maximum rows reproduce the blocked frontier and 10 settings reproduce the blocked March review, including per-trade account assignments; none of the replayed PA histories held two trades at once. The paired chart is not drawn here: it needs ReportLab, which the venv lacks.'),
