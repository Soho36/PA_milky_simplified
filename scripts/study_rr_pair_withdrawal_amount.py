...
their own variant's signals while flat. Assignment is fixed by purchase index,
...
OUT = ROOT / 'results/legacy_25k/rr_pair_monthly_amount_x_cushion'
PROFILE = ROOT / 'config/studies/legacy_25k.json'
CONFIG = TAPES = TRADES = TRADE_RR = None
ORDER = ('0.50', '2.50')


class AlternatingRouter(RRRouter):
    def assign(self, accounts):
        self.accounts = accounts
        order = tuple(self.weights)
        for a in accounts:
            self.account_rr[a.account_id] = order[(a.account_id - 1) % len(order)]
...
def initialize(order=ORDER):
    ...
    CONFIG = replace(base, risk_reward='0.50/2.50', expected_trades=None,
                     scenario='rr_pair_monthly_amount_x_cushion',
                     brick_name='rr_pair_monthly_amount_x_cushion')
...
def simulate(policy, cushion):
    ...
    return run_book(TRADES, replace(CONFIG, policy=p), routing=RoutingPolicy(mode='blocked'),
                    router_factory=partial(AlternatingRouter, weights=dict.fromkeys(ORDER, 1),
                                           trade_rr=TRADE_RR))


def audit_result(result):
    """Audit each accepted copy, including actual fill identities and occupancy."""
    accounts = {a.account_id: a for a in result.accounts}
    assigned = result.routing['account_rr']
    last_exit, keys = {}, {i: [] for i in accounts}
    for fill in result.routing_fills:
        ...
        for i in fill['accounts']:
            ...
            assert i not in last_exit or last_exit[i] <= entry
            last_exit[i] = exit_at
...
def main():
    ...
    payload = dict(schema='pa_milky.rr_pair_amount_cushion.v1',
                   ...
                   scripts={p.name:sha256_file(p) for p in (Path(__file__), ROOT/'scripts/study_withdrawal_amount.py', ROOT/'scripts/report_names.py')},
                   ...
                   design=dict(assignment='alternate monthly purchase index; fixed for life', order=ORDER,
                               ...
                               execution='all eligible own-variant signals while flat; one MNQ per account',
    ...
    report = '# RR 0.50 / RR 2.50: monthly withdrawal amount x cushion\n\n'
    report += (f'{len(rows)} candidates including hold. Same eight payout families, coarse grid and local refinement as the original study. '
               'Full historical payout rulebook, processing delay off, one MNQ per account, $200 purchase fee.\n\n'
               ...
               '**Execution difference:** each account accepts every eligible signal from its variant while flat. '
               'The referenced original study allowed overlapping positions; its dollar totals are not a matched control. '
    ...
    report += original.ceiling_section(rows, benchmark)
    ...
    report += original.findings(rows)
    report += ('\n## Validation and reproduction\n\n'
               f'All {len(rows)} economic ledgers reconcile. Audited {sum(r["copies_checked"] for r in rows):,} accepted copies '
               'for variant membership, activation, non-overlap, counts and killing trades. Both winning runs were reproduced with account and payout ledgers. '
               ...
               'Reproduce: `venv/Scripts/python.exe scripts/study_rr_pair_withdrawal_amount.py --workers 4`. '
